...
class GithubRepoAdapterIssueTest < GithubRepoAdapterBaseTest
  def setup
    super
    @test_issue = nil
  end
  ...
  def teardown
    ...
    if @test_issue && @test_issue.key
      VCR.use_cassette("github_repo_adapter/teardown_close_issue_#{@test_issue.key}") do
  ...
  def test_list_issues
    VCR.use_cassette("github_repo_adapter/list_issues") do
      ...
      issues.each do |issue|
        ...
        assert_equal :github_repo, issue.adapter_source
  ...
  def test_list_issues_with_status_filter
    VCR.use_cassette("github_repo_adapter/list_issues_with_status_filter") do
  ...
  def test_list_issues_with_pagination
    VCR.use_cassette("github_repo_adapter/list_issues_with_pagination") do
  ...
  def test_find_issue
    ...
    VCR.use_cassette("github_repo_adapter/create_issue_for_find_test") do
      @test_issue = create_test_issue("for_find_test")
      assert @test_issue, "Failed to create test issue"
    end

    # Now find the issue
    VCR.use_cassette("github_repo_adapter/find_issue") do
      ...
      assert_equal :github_repo, found_issue.adapter_source
    end
  end

  def test_find_issue_not_found
    VCR.use_cassette("github_repo_adapter/find_issue_not_found") do
  ...
  def test_create_issue
    VCR.use_cassette("github_repo_adapter/create_issue") do
  ...
  def test_create_issue_with_assignees
    VCR.use_cassette("github_repo_adapter/create_issue_with_assignees") do
  ...
  def test_update_issue_title
    # Create a test issue first
    VCR.use_cassette("github_repo_adapter/create_issue_for_update_test") do
      @test_issue = create_test_issue("for_update_test")
      assert @test_issue, "Failed to create test issue"
    end

    # Update the issue title
    VCR.use_cassette("github_repo_adapter/update_issue_title") do
  ...
  def test_update_issue_status
    # Create a test issue first
    VCR.use_cassette("github_repo_adapter/create_issue_for_status_update_test") do
    ...
    # Close the issue
    VCR.use_cassette("github_repo_adapter/update_issue_close") do
    ...
    # Reopen the issue
    VCR.use_cassette("github_repo_adapter/update_issue_reopen") do
  ...
  def test_update_issue_assignees
    # Create a test issue first
    VCR.use_cassette("github_repo_adapter/create_issue_for_assignee_update_test") do
      @test_issue = create_test_issue("for_assignee_update_test")
      assert @test_issue, "Failed to create test issue"
    end

    # Get authenticated user for assignee
    VCR.use_cassette("github_repo_adapter/get_user_for_assignee_update") do
      current_user = @adapter.get_current_user

      # Add assignee
      VCR.use_cassette("github_repo_adapter/update_issue_add_assignee") do
        updated_issue = @adapter.update_issue(@test_issue.key, {
      ...
      # Remove assignee
      VCR.use_cassette("github_repo_adapter/update_issue_remove_assignee") do
        updated_issue = @adapter.update_issue(@test_issue.key, {
          assignees: []
        })
  ...
  def test_delete_issue
    # Create a test issue first
    VCR.use_cassette("github_repo_adapter/create_issue_for_delete_test") do
    ...
    VCR.use_cassette("github_repo_adapter/delete_issue") do
  ...
  def test_issue_factory
    VCR.use_cassette("github_repo_adapter/issue_factory") do
      # Create a test issue first
