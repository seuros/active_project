...
class OrmInterfaceTest < ActiveSupport::TestCase
  def setup
    ...
    ActiveProject.configure do |config|
      ...
      config.add_adapter :github, :primary, owner: "owner", repo: "repo", access_token: "token"
    end
    ...
    @dummy_github_repo = ActiveProject::Resources::Project.new(@github_adapter, id: "repo1", name: "GitHub Repo",
                                                                              key: "owner/repo", adapter_source: :github)
    @dummy_github_issue = ActiveProject::Resources::Issue.new(@github_adapter, id: "issue1", key: "1",
    ...
    @github_adapter.stubs(:list_projects).with({}).returns([ @dummy_github_repo ])
    @github_adapter.stubs(:find_project).with("owner/repo").returns(@dummy_github_repo)
    @github_adapter.stubs(:list_issues).with("owner/repo", {}).returns([ @dummy_github_issue ])
    @github_adapter.stubs(:find_issue).with("1").returns(@dummy_github_issue)
    @github_adapter.stubs(:create_issue).with(has_entries(title: "Create Test")).returns(@dummy_github_issue)
  end
  ...
  test "adapter responds to factory methods" do
    ...
    assert_respond_to @github_adapter, :projects
    assert_respond_to @github_adapter, :issues
  end
